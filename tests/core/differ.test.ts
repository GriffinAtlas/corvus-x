...
describe('diffSnapshots', () => {
  ...
  it('matches array items by compound key (string[])', () => {
    const oldData = {
      peakWindows: [
        { day: 'Monday', startHour: 9, endHour: 12, score: 0.8 },
        { day: 'Wednesday', startHour: 14, endHour: 17, score: 0.6 },
      ],
    }
    const newData = {
      peakWindows: [
        { day: 'Monday', startHour: 9, endHour: 12, score: 0.9 },
        { day: 'Wednesday', startHour: 14, endHour: 17, score: 0.6 },
      ],
    }
    const lines = diffSnapshots(oldData, newData, { peakWindows: ['day', 'startHour'] })

    // Monday|9 changed score from 0.8 to 0.9
    expect(lines).toHaveLength(1)
    expect(lines[0].type).toBe('changed')
    expect(lines[0].path).toContain('Monday|9')
    expect(lines[0].path).toContain('score')
    expect(lines[0].oldValue).toBe('0.80')
    expect(lines[0].newValue).toBe('0.90')
  })

  it('detects added items with compound key', () => {
    const oldData = {
      peakWindows: [
        { day: 'Monday', startHour: 9, endHour: 12, score: 0.8 },
      ],
    ...
    const newData = {
      peakWindows: [
        { day: 'Monday', startHour: 9, endHour: 12, score: 0.8 },
        { day: 'Friday', startHour: 16, endHour: 19, score: 0.7 },
      ],
    }
    const lines = diffSnapshots(oldData, newData, { peakWindows: ['day', 'startHour'] })
    ...
    expect(lines[0].newValue).toContain('Friday')
  })

  it('detects removed items with compound key', () => {
    const oldData = {
      peakWindows: [
        { day: 'Monday', startHour: 9, endHour: 12, score: 0.8 },
        { day: 'Friday', startHour: 16, endHour: 19, score: 0.7 },
    ...
    expect(lines[0].oldValue).toBe('Friday|16')
  })

  it('returns no diff when compound-keyed items are identical', () => {
    const data = {
      peakWindows: [
        { day: 'Monday', startHour: 9, endHour: 12, score: 0.8 },
      ],
    }
    const lines = diffSnapshots(data, data, { peakWindows: ['day', 'startHour'] })
    expect(lines).toEqual([])
  })

  it('handles compound key with missing field gracefully', () => {
    const oldData = {
      items: [{ a: 'x', b: undefined, val: 1 }],
    }
    const newData = {
      items: [{ a: 'x', b: undefined, val: 2 }],
    }
    // compound key references 'b' which is undefined — should still match via 'x|undefined'
    const lines = diffSnapshots(oldData, newData, { items: ['a', 'b'] })
    expect(lines).toHaveLength(1)
    expect(lines[0].type).toBe('changed')
  })
})
